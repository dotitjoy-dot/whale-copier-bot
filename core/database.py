...
class Database:
    ...
    async def add_wallet(
        self, telegram_id: int, chain: str, address: str, encrypted_pk: str, label: str = "Main"
    ) -> int:
        """Store a new encrypted wallet. Returns the new wallet id."""
        cursor = await self._conn.execute(
            """INSERT INTO wallets (telegram_id, chain, address, encrypted_pk, label)
               VALUES (?, ?, ?, ?, ?)""",
            (telegram_id, chain, address, encrypted_pk, label),
        )
        await self._conn.commit()
